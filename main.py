...
def main():
    ...
    # Example usage
    m = 21082.41  # Example meters value
    R = 35.0  # Example VDOT value
    DISTANCES = {
        "1K": 1000,
        "1M": 1609.34,
        "3K": 3000,
    ...
    table = list()
    for v in range(start_int, end_int+1, step_int):
        temp = list()
        v_float = v/10
        temp.append(v_float)
    ...
    out = pd.DataFrame(table, columns=cols)
    print(out.head())
    

    # try:
    #     root = newton_raphson(m, R)
    #     print(f"Root found: t = {root}")
        
    #     print(format_minutes(root))
